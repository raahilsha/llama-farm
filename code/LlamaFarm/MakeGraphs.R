...
for (csvfile in toanalyze)
{
	...
	newfilen <- paste(substring(csvfile, 0, which(strsplit(csvfile, "")[[1]]==".")[1] - 1), "_ticks.png", sep = "")
	...
	newfilen <- paste(substring(csvfile, 0, which(strsplit(csvfile, "")[[1]]==".")[1] - 1), "_genes.png", sep = "")
	png(filename=newfilen,width=512,height=512)
